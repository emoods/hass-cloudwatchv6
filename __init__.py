"""Update the IP addresses of your Cloudflare DNS records incl V6"""
...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    async def update_records(now):
        ...
        try:
            await _async_update_cloudflare(hass, cfupdate, zone_id)
...
async def _async_update_cloudflare(
    hass: HomeAssistant, cfupdate: CloudflareUpdater, zone_id: str
):
    _LOGGER.debug("Starting update for zone %s", cfupdate.zone)

    for idx, source_ip in enumerate(await _async_build_source_set(hass)):
        _LOGGER.info(idx, source_ip)
